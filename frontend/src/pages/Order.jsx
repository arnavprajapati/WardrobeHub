...
const Order = () => {
    ...
    const loadOrderData = async () => {
        ...
        try {
            ...
            const userOrders = result.data.orders || result.data;
            ...
            <div className="max-w-7xl mx-auto w-full">
                ...
                {orders.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-6 max-h-[calc(100vh-200px)] overflow-y-auto">
                        {orders.map((order) => (
                            <div key={order._id} className="bg-gray-100 rounded-xl shadow-md p-4 sm:p-6 hover:shadow-lg transition-shadow duration-200">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b-2 pb-4 mb-4">
                                    <h3 className="text-lg font-bold text-black">Order ID: <span className="font-normal">{order._id.slice(-6)}</span></h3>
                                    <p className="text-sm text-gray-500">
                                        Order Date: <span className="text-black font-medium">{new Date(order.date).toLocaleDateString('en-IN')}</span>
                                    </p>
                                    <p className="text-sm font-semibold text-black">
                                        Total Amount: <span className="text-lg text-purple-600">{currency}{order.amount}</span>
                                    </p>
                                </div>
                                
                                {order.items.map((item, index) => (
                                    <div key={index} className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-6 mb-4 last:mb-0">
                                        ...
                                        <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-6 mt-4 lg:mt-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-black font-bold text-sm min-w-[100px]">{order.status || 'Order Placed'}</p>
                                            </div>
                                            
                                            <div className="flex flex-col items-start lg:items-center min-w-[120px]">
                                                <p className="text-xs text-gray-700">Payment</p>
                                                <p className={`text-sm font-semibold ${getPaymentStatusColor(order.payment, order.paymentMethod)}`}>
                                                    {getPaymentStatusText(order.payment, order.paymentMethod)}
                                                </p>
                                            </div>

                                            <button
                                                onClick={loadOrderData}
                                                className="px-4 py-2 text-sm font-semibold rounded-lg text-white bg-black hover:bg-gray-800 transition-colors duration-200 flex-shrink-0 cursor-pointer"
                                            >
                                                Refresh Orders
                                            </button>
                                        </div>
